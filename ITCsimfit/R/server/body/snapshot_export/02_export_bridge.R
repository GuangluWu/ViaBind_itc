  ...
  build_fit_export_bundle <- function(sim = NULL) {
    ...
    meta_rev <- export_bridge_build_meta_rev(
      ...
      meta_updates = list(
        ...
        generated_by = export_bridge_build_version_signature("ITCsimfit")
      )
    )
    ...
    report_text <- tryCatch(
      ...
      error = function(e) {
        paste0(
          "FITTING REPORT\n",
          "Generated by ", export_bridge_build_version_signature("ITCsimfit"), " on ", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n",
