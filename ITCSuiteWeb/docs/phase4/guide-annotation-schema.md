# ITCSuiteWeb Phase 4: 用户引导注释配置 Schema

## 1. 目标

为 `ITCSuiteWeb` 预埋“按控件 id 绑定文案”的配置能力，当前阶段不渲染 UI。
采用独立 CSV，不与其他子应用翻译表混用。

## 2. 版本

1. 当前 schema：`itcsuite.guide_annotation.v1`
2. 版本字段：`schema_version`
3. 未来升级使用 `v2/v3`，避免隐式破坏变更
...
7. 若 `since_version` 和 `until_version` 同时存在，要求 `since <= until`
...
1. 仅返回 `status=active`
2. `app` 匹配 `ITCSuiteWeb` 或 `all`
3. 如传入 `app_version`，按版本区间过滤
