...
testthat::test_that("ITCprocessor guide config passes baseline validation", {
  df <- load_guide_annotations(file.path(proc_dir, "config", "guide_annotations.v1.csv"))
...
testthat::test_that("validator rejects missing columns and invalid enums", {
  ...
  df2 <- load_guide_annotations(file.path(proc_dir, "config", "guide_annotations.v1.csv"))
