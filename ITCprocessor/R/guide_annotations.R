...
GUIDE_ALLOWED_CONTROL_TYPES <- c(
  ...
  "textinput", "plotoutput", "plotlyoutput", "dtoutput", "uioutput", "selectinput", "other"
)
...
resolve_guide_annotations_path <- function(path) {
  ...
  candidates <- unique(unlist(lapply(dirs, function(d) {
    c(file.path(d, raw), file.path(d, "ITCprocessor", raw))
  }), use.names = FALSE))
