...
# 优化后的分段样条算法 (Advanced Segmented Spline)
# 逻辑改进：
# 1. 严格使用传入的 injection_indices (来自文件解析)，不再自己猜
# 2. 提供更多可控性：用户可以手动调整锚点的位置逻辑

# 内部辅助函数：给定结束索引，寻找符合时间长度的区间并计算均值
...
SegmentedBaseline <- function(time, power, injection_indices,
  ...
  if (length(anchors_x) > 1) {
    if (is.null(spar) || is.na(spar)) {
      fit_spline <- smooth.spline(anchors_x, anchors_y)
    } else {
      fit_spline <- smooth.spline(anchors_x, anchors_y, spar = spar)
    }
...
SegmentedBaselineAnchors <- function(time, power, injection_indices,
  ...
  data.frame(x = anchors_x, y = anchors_y)
}
